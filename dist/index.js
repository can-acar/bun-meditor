...
// node_modules/bun-cancellation-token/dist/index.js
...
// src/mediator.ts
var MEDIATOR = token("core:Mediator");
...
class Mediator {
  ...
  useBehavior(b) {
    ...
    this.behaviors.sort((a, b2) => (a.order ?? 100) - (b2.order ?? 100));
  }
  useMetrics(sink) {
    this.metrics = sink;
  }
  registerRequest(key, handlerToken) {
    const arr = this.req.get(key) ?? [];
    arr.push({ kind: "token", token: handlerToken });
    this.req.set(key, arr);
  }
  registerNotification(key, handlerToken) {
    const arr = this.evt.get(key) ?? [];
    arr.push({ kind: "token", token: handlerToken });
    this.evt.set(key, arr);
  }
  registerCallable(key, kind, ref) {
    const map = kind === "request" ? this.req : this.evt;
    const arr = map.get(key) ?? [];
    arr.push(ref);
    map.set(key, arr);
  }
  scanHandles(instance) {
    const ctor = instance.constructor;
    const list = Reflect.getOwnMetadata("mediator:handles", ctor) ?? [];
    for (const it of list) {
      const fn = instance[it.method].bind(instance);
      const ref = { kind: "callable", fn: async (payload, ctx) => fn(payload, ctx) };
      this.registerCallable(it.key, it.kind, ref);
    }
  }
  async send(key, req, options = {}) {
    ...
    if (!ref)
      throw new HandlerNotFoundError(key);
    const handlerInvoke = async () => {
      if (ref.kind === "token") {
        const h = this.container.resolve(ref.token);
        return await h.handle(req, ctx);
      } else {
        return await ref.fn(req, ctx);
      }
    };
    const invokeWithBehaviors = this.behaviors.reduceRight((next, b) => () => b.handle(req, next, ctx), handlerInvoke);
    this.metrics?.onRequestStart(key, ctx.meta);
    ...
    try {
      ...
      if (ct.isCancellationRequested)
        throw new CanceledError;
      this.metrics?.onRequestOk(key, performance.now() - started, ctx.meta);
      return result;
    } catch (e) {
      this.metrics?.onRequestError(key, performance.now() - started, e, ctx.meta);
      throw e;
    } finally {
      cts?.token.onCancelled(() => {});
    }
  }
  async publish(key, evt, options = {}) {
    const { timeoutMs, ct: parent, meta = {} } = options;
    const base = parent ?? new CancellationToken(new AbortController().signal);
    const cts = timeoutMs != null ? withTimeout(timeoutMs, base) : undefined;
    const ct = cts ? anyToken(base, cts.token) : base;
...
(function(Reflect4) {
  (function(factory) {
    var root = typeof global === "object" ? global : typeof self === "object" ? self : typeof this === "object" ? this : Function("return this;")();
    var exporter = makeExporter(Reflect4);
    if (typeof root.Reflect === "undefined") {
      root.Reflect = Reflect4;
    } else {
      exporter = makeExporter(root.Reflect, exporter);
    }
    factory(exporter);
...
var defaultPolicy2 = ({ candidates, requestedName, ctx }) => {
  const filtered = candidates.filter((r) => holds2(r.when, ctx));
  if (requestedName) {
    const exact = filtered.find((r) => r.name === requestedName);
    return exact ?? undefined;
  }
...
class Container2 {
  ...
  register(token2, factory, lifetime = "transient", name, when) {
    const key = this.mapKey(token2, name);
    const arr = this.map.get(key) ?? [];
    arr.push({ lifetime, factory, name, when });
    this.map.set(key, arr);
    return this;
  }
  registerAsync(token2, asyncFactory, lifetime = "transient", name, when) {
    const key = this.mapKey(token2, name);
    const arr = this.map.get(key) ?? [];
    arr.push({ lifetime, asyncFactory, name, when });
    this.map.set(key, arr);
  ...
  whenEnv(token2, key, value, factory, lifetime = "singleton", name) {
    return this.register(token2, factory, lifetime, name, { type: "env", key, equals: value });
  }
  whenFlag(token2, key, expected, factory, lifetime = "singleton", name) {
    return this.register(token2, factory, lifetime, name, { type: "flag", key, value: expected });
  }
  whenProfile(token2, profile, factory, lifetime = "singleton", name) {
    return this.register(token2, factory, lifetime, name, { type: "profile", name: profile });
  }
  when(token2, pred, factory, lifetime = "singleton", name) {
    return this.register(token2, factory, lifetime, name, { type: "fn", fn: pred });
  }
  resolve(token2, name) {
    const reg = this.pickRegistration(token2, name);
    if (!reg)
      throw new Error(`DI: ambiguous or missing ${token2.toString()} [${name ?? "auto"}]`);
    if (reg.asyncFactory)
      throw new Error(`DI: ${token2.toString()} is async, use resolveAsync()`);
    if (reg.resolving)
      throw new Error(`Circular dependency detected for ${token2.toString()} [${reg.name ?? "auto"}]`);
    try {
      reg.resolving = true;
      if (reg.lifetime !== "transient") {
        if (reg.instance === undefined)
  ...
  async resolveAsync(token2, name) {
    const reg = this.pickRegistration(token2, name);
    if (!reg)
      throw new Error(`DI: ambiguous or missing ${token2.toString()} [${name ?? "auto"}]`);
    if (reg.resolving)
    ...
    try {
      ...
      return this.resolve(token2, name);
    } finally {
      reg.resolving = false;
    }
  }
  resolveAll(token2) {
    const regs = this.listVariants(token2).filter((r) => holds2(r.when, this.ctx));
    if (regs.length === 0 && this.parent)
      return this.parent.resolveAll(token2);
    return regs.map((r) => {
      if (r.asyncFactory)
        throw new Error(`resolveAll: async binding present, use resolveAllAsync`);
      if (r.lifetime !== "transient") {
  ...
  async resolveAllAsync(token2) {
    const regs = this.listVariants(token2).filter((r) => holds2(r.when, this.ctx));
    if (regs.length === 0 && this.parent)
      return await this.parent.resolveAllAsync(token2);
    const out = [];
    for (const r of regs) {
      if (r.asyncFactory) {
        if (r.lifetime !== "transient") {
        ...
        out.push(this.resolve(token2, r.name));
      }
    }
    return out;
  }
  resolveLazy(token2, name) {
    const lock = new LazyLock2(() => this.resolve(token2, name));
    const sample = this.pickRegistration(token2, name, true);
    if (!sample)
      throw new Error(`DI: ambiguous or missing ${token2.toString()} [${name ?? "auto"}]`);
    return () => sample.lifetime === "transient" ? this.resolve(token2, name) : lock.getSync();
  }
  resolveLazyAsync(token2, name) {
    const lock = new LazyLock2(() => this.resolveAsync(token2, name));
    const sample = this.pickRegistration(token2, name, true);
    if (!sample)
      throw new Error(`DI: ambiguous or missing ${token2.toString()} [${name ?? "auto"}]`);
    return async () => sample.lifetime === "transient" ? await this.resolveAsync(token2, name) : await lock.get();
  }
  injectLazy(token2, name) {
    const lock = new LazyLock2(() => this.resolve(token2, name));
    const sample = this.pickRegistration(token2, name, true);
    if (!sample)
      throw new Error(`DI: ambiguous or missing ${token2.toString()} [${name ?? "auto"}]`);
    return {
      get: () => sample.lifetime === "transient" ? this.resolve(token2, name) : lock.getSync(),
      enumerable: true,
      configurable: true
    };
  }
  createScope() {
    const child = new Container2(this);
    child.setEnv(this.ctx.env);
    child.setFlags(this.ctx.flags);
    child.setProfile(this.ctx.profile);
    child.setResolutionPolicy(this._policy);
  ...
  pickRegistration(token2, requestedName, searchParent = false) {
    const variants = this.listVariants(token2);
    if (variants.length === 0 && this.parent && searchParent !== false)
      return this.parent.pickRegistration(token2, requestedName, true);
    return this._policy({ token: token2, candidates: variants, requestedName, ctx: this.ctx });
  }
}
var token2 = (desc) => Symbol.for(desc);

// src/decorators.ts
function RequestKey(key) {
  return function(target) {
    Reflect.defineMetadata("mediator:reqkey", key, target);
  };
}
function NotificationKey(key) {
  return function(target) {
    Reflect.defineMetadata("mediator:notifkey", key, target);
  };
}
function Handle(opts) {
  return function(target, propertyKey, descriptor) {
    const list = Reflect.getOwnMetadata("mediator:handles", target.constructor) ?? [];
    list.push({ method: propertyKey, key: opts.key, kind: opts.kind });
    Reflect.defineMetadata("mediator:handles", list, target.constructor);
  };
}
function asHandler(key, kind, fn) {
  return { kind: "callable", fn: async (p, ctx) => fn(p, ctx) };
}
var REQUEST = (name) => token2(`req:${name}`);
var NOTIF = (name) => token2(`evt:${name}`);
export {
  asHandler,
  TimeoutBehavior,
  RetryBehavior,
